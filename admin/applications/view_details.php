<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT a.*,c.name as club, CONCAT(a.lastname,', ',a.firstname,' ', COALESCE(a.middlename,'')) as `name`, CONCAT(a.student_id) as 'student_id', CONCAT(a.year_level,' - ',a.section) as `class` FROM `application_list` a inner join club_list c on a.club_id =c.id where a.id = '{$_GET['id']}'");
    if($qry->num_rows > 0 ){
...
<section class="py-4">
    <div class="container">
        <dl class="row">
            ...
            <dt class="col-4 border py-1">Student ID</dt>
            <dd class="col-8 px-2 border py-1 mb-0"><?= isset($student_id) ? $student_id : "" ?></dd>
            <dt class="col-4 border py-1">Batch - Degree</dt>
