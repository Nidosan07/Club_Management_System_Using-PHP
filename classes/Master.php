...
Class Master extends DBConnection {
	...
	function dt_applications(){
		...
		$columns_arr = array("unix_timestamp(a.date_updated)",
							...
							"a.student_id",
							"a.status");
		$query = $this->conn->query("SELECT a.*, CONCAT(a.lastname,', ',a.firstname,' ', COALESCE(a.middlename,'')) as `name`, CONCAT(a.year_level,' - ',a.section) as `class`, c.name as club, a.student_id as student_id FROM `application_list` a inner join club_list c on a.club_id = c.id  {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start} ");
...
function saveEvent() {
    ...
    if (empty($status)) {
        echo json_encode(array('status' => 'error', 'msg' => 'Status is required.'));
        exit;
    }

    // Insert or update data into the event_list table
    try {
        if ($id) {
            // Update existing record
            $conn->query("UPDATE `event_list` SET status = '$status' WHERE id = '$id'");
        } else {
            // Insert new record
            $conn->query("INSERT INTO `event_list` (status) VALUES ('$status')");
        }

        echo json_encode(array('status' => 'success'));
    } catch (Exception $e) {
        echo json_encode(array('status' => 'error', 'msg' => 'An error occurred while saving data.'));
    }
}
...
function deleteEvent() {
    ...
    if (empty($id) || !is_numeric($id)) {
        echo json_encode(array('status' => 'error', 'msg' => 'Invalid event ID.'));
        exit;
    }

    // Delete the record from the event_list table
    try {
        $conn->query("DELETE FROM `event_list` WHERE id = '$id'");
        echo json_encode(array('status' => 'success'));
    } catch (Exception $e) {
        echo json_encode(array('status' => 'error', 'msg' => 'An error occurred while deleting the event.'));
    }
}